    ...
    Vertex source)
{
    ...
    while (again) {
        again = false;
        for (Vertex u = 0; u < _g.num_vertices(); ++u) {
            ...
            _valid[u] = 1;     // u now processed
        }
    }
    build_tree_from_parents();     // update subtree lists for next batch
}
